...
from ml_nexus.schematics import ResolveMountRequest, CacheMountRequest, ContainerSchematic, \
    ContextualMountRequest, MountRequest
...
@injected
@beartype
async def build_entrypoint_script(
        scripts: list[str]
):
    assert isinstance(scripts, list), f"scripts must be a list of strings. got {type(scripts)}"
    joined_script = '\n'.join(scripts)
    entrypoint_script = f"""
#!/bin/bash
echo "#########################"
...
async def docker_builder__for_rye_v2(
    ...
    """
    assert target.dirs[
               0].kind == 'rye', f"the first dir of the project must be rye. got {target.dirs[0].kind},{target.dirs[0].id}"
    ...
    return new_DockerBuilder(
        ...
        macros=[
            ...
            await macro_preinstall_from_requirements_with_rye(base_image, target, pyproject_dir_in_container),
...
async def a_get_mount_request_for_pdir(
    ...
    @asynccontextmanager
    async def rye_impl():
        async with patch_rye_project(tgt=pdir, source_root=placement.sources_root) as patched_pdir:
            yield patched_pdir
    ...
    match kind:
        ...
        case 'uv':
            uv_request = ContextualMountRequest(
                source=uv_impl, mount_point=placement.sources_root / pdir.id,
                excludes=pdir.excludes)
            return uv_request
        case 'rye':
...
@injected
async def a_gather_mount_request_for_project(a_get_mount_request_for_pdir, /, project: ProjectDef) \
        -> tuple[MountRequest]:
    return await asyncio.gather(*[a_get_mount_request_for_pdir(placement=project.placement, pdir=pdir) for pdir in
                                  project.yield_project_dirs()])
