...
class DockerEnvFromSchematics(IScriptRunner):
    ...
    async def _rsync_mount(self, source, host_dst, mount_point, excludes):
        ...
        await rsync.run()
        return DockerMount(source=host_dst, target=mount_point)
    ...
    async def prepare_mounts(self):
        ...
        async def task_impl(cmr: ContextualMountRequest):
            async with cmr.source() as host_src:
                res = await self.sync_direct(
                    cmr.excludes,
                    cmr.mount_point,
                    host_src,
                    unique_id=str(cmr.mount_point),
                )
                return res
            ...
            for mount in self.schematics.mount_requests:
                match mount:
                    ...
                    case ResolveMountRequest(
                        ...
                        mount_task = tg.create_task(
                            self._rsync_mount(
                                local_path, host_dst, mount_point, excludes
                            )
                        )
                        # 2. mount
                    ...
                    case DirectMountRequest(source, mount_point, excludes):
                        mount_task = tg.create_task(
                            self.sync_direct(excludes, mount_point, source)
                        )
                        mount_tasks.append(mount_task)
                    case _:
