...
from pinjected import IProxy, design, instance
...
test_design = design(
    ...
    ml_nexus_docker_build_context="zeus-context"
)


@injected_pytest(test_design)
async def test_docker_client_integration(ml_nexus_default_docker_client, mock_a_system, logger):
    ...
    assert any("docker --context zeus-context run" in call for call in calls), f"Context not found in run command. Calls: {calls}"
    assert any("docker --context zeus-context push" in call for call in calls), f"Context not found in push command. Calls: {calls}"
...
async def test_build_operations_with_context(a_build_docker, mock_a_system, logger):
    ...
    assert any("docker --context zeus-context build" in call for call in calls), f"Context not found in build. Calls: {calls}"
    assert any("docker --context zeus-context push" in call for call in calls), f"Context not found in push. Calls: {calls}"
