...
async def test_container_persistence_across_instances(
    ...
    try:
        ...
        result = await docker_env1.run_script(
            "echo 'persistent data' > /tmp/test_file.txt"
        )
        assert result.exit_code == 0
...
async def test_file_operations(
    ...
    try:
        ...
        result = await docker_env.run_script(
            f"test -f {remote_path} && echo 'exists' || echo 'deleted'"
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            await docker_env.run_script(
                f"echo 'Modified in container' > {remote_sync_path}/file3.txt"
            )
...
async def test_docker_context_support(
    ...
    try:
        ...
        assert container_name in df.index, (
            f"Container {container_name} not found in docker ps"
        )
...
async def test_error_handling(
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            result = await docker_env.run_script(
                "test -f /deep/nested/path/file.txt && echo 'exists'"
            )
...
async def test_python_execution_uv(
    ...
    try:
        ...
        result = await docker_env.run_script(
            "python -c 'print(\"Hello from persistent container!\")'"
        )
...
async def test_python_execution_requirements_auto(
    ...
    logger.info(
        "Testing Python execution in persistent container with requirements.txt (auto detection)"
    )
    ...
    schematic = await schematics_universal(
        target=project,
        base_image="ubuntu:22.04",  # Use Ubuntu base to test pyenv installation
        python_version="3.11",
    )
...
'"""
        ...
        result = await docker_env.run_script(
            "ls -la $HOME/.pyenv/versions/ 2>/dev/null || echo 'No pyenv'"
        )
