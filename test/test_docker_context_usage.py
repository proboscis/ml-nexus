...
import pytest
from pinjected import design, instance, IProxy
...
async def test_local_docker_client_uses_context(new_LocalDockerClient, mock_a_system):
    ...
    mock_a_system.assert_called_with(
        "docker --context test-context run -d test-image echo hello"
    )

    # Test exec_container
    await client.exec_container("test-container", "ls -la")
    mock_a_system.assert_called_with(
        "docker --context test-context exec test-container ls -la"
    )

    # Test build_image
    await client.build_image(Path("/tmp/context"), "test:latest", ["-f", "Dockerfile"])
    mock_a_system.assert_called_with(
        "docker --context test-context build -f Dockerfile -t test:latest /tmp/context"
    )
    ...
    mock_a_system.assert_called_with(
        "docker --context test-context stop test-container"
    )
...
async def test_docker_build_functions_use_context(mock_a_system):
    ...
    await a_build_docker(
        tag="test:latest", context_dir="/tmp/context", options="--no-cache", push=True
    )

    # Verify build command uses context
    assert any(
        "docker --context test-context build" in str(call)
        for call in mock_a_system.call_args_list
    )
    # Verify push command uses context
    assert any(
        "docker --context test-context push" in str(call)
        for call in mock_a_system.call_args_list
    )
...
@injected_pytest(test_design)
async def test_docker_push_local_uses_context(
    mock_a_system, mock_a_setup_docker_credentials
):
...
"""
    ...
    await build_image_with_rsync(code=dockerfile_content, tag="test:latest", push=True)
...
async def test_docker_operations_with_empty_context(
    new_LocalDockerClient, mock_a_system
):
